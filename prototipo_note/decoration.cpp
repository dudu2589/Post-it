...
Decoration::Decoration(int color , QWidget* parent)
    : QWidget(parent) {
    setcolor(color);
    ...
    setLayout(layout);
    connect(quit, &QPushButton::clicked,(const QWidget*) parent->parent(), &QWidget::close);
    connect(add, &QPushButton::clicked,this, &Decoration::on_add_clicked);
}
...
void Decoration::on_add_clicked() {
    Sticky* sticky = new Sticky(color_);
    sticky->show();
}
